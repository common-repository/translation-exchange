...
if (!file_exists(get_option('tml_cache_path'))) {
    mkdir(get_option('tml_cache_path'), 0700);
}
    ...
    foreach ($snapshots as $snapshot) {
        ...
        foreach ($snapshot['application']['languages'] as $language) {
            ...
            $progress[$language['locale']] = array(
                'translated' => $total_keys > 0 ? round($translated_keys / $total_keys * 100) : 0,
                'approved' => $total_keys > 0 ? round($approved_keys / $total_keys * 100) : 0
            );
        ...
        <div style="border: 1px solid #ccc; width: 100%; margin-bottom: 10px; border-radius: 5px; background: #eee;">
            ...
            <div id="snapshot-<?php echo $snapshot['version'] ?>-details"
                 style="display: none; padding:10px; border-top: 1px solid #ccc; background: white; border-bottom-right-radius: 5px; border-bottom-left-radius: 5px;">
