...
/**
 * Converts PO to MO file
 *
 * @param $input
 * @param bool|false $output
 * @return bool
 */
function trex_po_to_mo($input, $output = false) {
    if ( !$output )
        $output = str_replace( '.po', '.mo', $input );
    $hash = trex_parse_po_file( $input );
    if ( $hash === false ) {
        return false;
    } else {
        trex_write_mo_file( $hash, $output );
        return true;
    }
}
 ...
 */
function trex_clean_helper($x) {
    if (is_array($x)) {
        foreach ($x as $k => $v) {
            $x[$k] = trex_clean_helper($v);
        }
    } else {
        if ($x[0] == '"')
            $x = substr($x, 1, -1);
 ...
 * @return array|bool
 */
function trex_parse_po_file($in) {
    // read .po file
    $fh = fopen($in, 'r');
    if ($fh === false) {
 ...
 * @param $out
 */
function trex_write_mo_file($hash, $out) {
    // sort by msgid
